...
trait VectorOps extends Variables {
  ...
  //syntax
  object Vector {
    def apply[A:Typ](length: Rep[Int]) = vectorNew(length)
  }
    
  implicit class VectorOpsCls[A:Typ](x: Rep[Vector[A]]) {
  ...
  //operations
  def vectorNew[A:Typ](length: Rep[Int]): Rep[Vector[A]]
  def vectorPlus[A:Typ:Numeric](x: Rep[Vector[A]], y: Rep[Vector[A]]): Rep[Vector[A]]
  def vectorPlusScalar[A:Typ:Numeric](x: Rep[Vector[A]], y: Rep[A]): Rep[Vector[A]]
  def scalarTimes[A:Typ:Numeric](x: Rep[Vector[A]], y: Rep[A]): Rep[Vector[A]]
  def vectorSum[A:Typ:Numeric](x: Rep[Vector[A]]): Rep[A]
  def vectorFilter[A:Typ](x: Rep[Vector[A]], pred: Rep[A] => Rep[Boolean]): Rep[Vector[A]]
  def vectorLength[A:Typ](x: Rep[Vector[A]]): Rep[Int]
  def vectorApply[A:Typ](x: Rep[Vector[A]], idx: Rep[Int]): Rep[A]
  def vectorPrint[A:Typ](x: Rep[Vector[A]]): Rep[Unit]
}
...
trait VectorOpsExp extends VectorOps with DeliteCollectionOpsExp with DeliteStructsExp {
  ...
  case class PPrint[A:Typ](x: Exp[Vector[A]], print: Block[Unit])
    extends DeliteOpSingleTask(print)
  
  //implemented via Delite ops
  case class VectorPlus[A:Typ:Numeric](inA: Exp[Vector[A]], inB: Exp[Vector[A]])
  ...
  abstract class VectorMap[A:Typ](in: Exp[Vector[A]]) extends DeliteOpMap[A,A,Vector[A]] {
    override def alloc(len: Exp[Int]) = Vector[A](len)
    val size = copyTransformedOrElse(_.size)(in.length)
  }

  case class VectorPlusScalar[A:Typ:Numeric](in: Exp[Vector[A]], s: Exp[A]) extends VectorMap[A](in) {
    def func = e => e + s
  }
  
  case class VectorTimesScalar[A:Typ:Numeric](in: Exp[Vector[A]], s: Exp[A]) extends VectorMap[A](in) {
    def func = e => e * s
  }
  
  case class VectorSum[A:Typ:Numeric](in: Exp[Vector[A]]) extends DeliteOpReduce[A] {
    val size = copyTransformedOrElse(_.size)(in.length)
    val zero = unit(0).AsInstanceOf[A]
    def func = (a,b) => a + b
  }
  
  case class VectorFilter[A:Typ](in: Exp[Vector[A]], cond: Exp[A] => Exp[Boolean]) extends DeliteOpFilter[A,A,Vector[A]] {
    val size = copyTransformedOrElse(_.size)(in.length)
    override def alloc(len: Exp[Int]) = Vector[A](len)    
    def func = a => a
  }

  case class VectorNew[A:Typ](len: Exp[Int]) extends DeliteStruct[Vector[A]] {
    val elems = copyTransformedElems(Seq("data" -> var_new(DeliteArrayBuffer[A](len)).e))
    val mA = manifest[A]
  }

  case class VectorNewImm[A:Typ](data: Exp[DeliteArray[A]], len: Exp[Int]) extends DeliteStruct[Vector[A]] {
    val elems = copyTransformedElems(Seq("data" -> DeliteArrayBuffer(data, len)))
    val mA = manifest[A]
  }


  def vectorNew[A:Typ](length: Exp[Int]) = reflectMutable(VectorNew(length))
  private def infix_data[A:Typ](x: Exp[Vector[A]]) = field[DeliteArrayBuffer[A]](x, "data")
  def vectorLength[A:Typ](x: Exp[Vector[A]]) = x.data.length
  def vectorApply[A:Typ](x: Exp[Vector[A]], idx: Exp[Int]) = x.data.apply(idx)
  def vectorPlus[A:Typ:Numeric](x: Exp[Vector[A]], y: Exp[Vector[A]]) = reflectPure(VectorPlus(x,y))
  def vectorPlusScalar[A:Typ:Numeric](x: Exp[Vector[A]], y: Exp[A]) = reflectPure(VectorPlusScalar(x,y))
  def scalarTimes[A:Typ:Numeric](x: Exp[Vector[A]], y: Exp[A]) = reflectPure(VectorTimesScalar(x, y))
  def vectorSum[A:Typ:Numeric](x: Exp[Vector[A]]) = reflectPure(VectorSum(x))
  def vectorFilter[A:Typ](x: Exp[Vector[A]], pred: Exp[A] => Exp[Boolean]) = reflectPure(VectorFilter(x,pred))
  def vectorPrint[A:Typ](x: Exp[Vector[A]]) = reflectEffect(PPrint(x, reifyEffectsHere(pprint_impl(x))))
