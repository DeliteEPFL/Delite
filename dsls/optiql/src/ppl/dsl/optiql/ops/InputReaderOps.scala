...
trait InputReaderOps extends Base { this: OptiQL =>
  ...
  object TableInputReader {
    ...
    def apply[T<:Record:Typ](path: Rep[String], separator: Rep[String]): Rep[Table[T]] = optiql_table_input_reader(path, separator)
  }

  def optiql_table_input_reader[T<:Record:Typ](path: Rep[String], separator: Rep[String]): Rep[Table[T]]
  def optiql_table_line_reader(path: Rep[String]): Rep[Table[String]]
  def optiql_table_from_seq[T:Typ](elems: Seq[Rep[T]]): Rep[Table[T]]
  def optiql_table_from_string[T<:Record:Typ](data: Rep[String], rowSep: Rep[String], colSep: Rep[String]): Rep[Table[T]]

}

trait InputReaderOpsExp extends InputReaderOps with BaseFatExp { this: OptiQLExp with InputReaderImplOps =>

  case class OptiQLTableInputReader[T:Typ](readBlock: Block[Table[T]]) extends DeliteOpSingleWithManifest[T,Table[T]](readBlock)
  case class OptiQLTableFromSeq[T:Typ](readBlock: Block[Table[T]]) extends DeliteOpSingleWithManifest[T,Table[T]](readBlock)

  def optiql_table_input_reader[T<:Record:Typ](path: Rep[String], separator: Rep[String]) = {
  ...
  def optiql_table_from_seq[T:Typ](elems: Seq[Rep[T]]) = {
  ...
  def optiql_table_from_string[T<:Record:Typ](data: Rep[String], rowSep: Rep[String], colSep: Rep[String]) = {
...
trait InputReaderImplOps { this: OptiQL =>
  def optiql_table_record_parser_impl[T<:Record:Typ](record: Rep[String], separator: Rep[String]): Rep[T]
  def optiql_table_from_seq[T:Typ](elems: Seq[Rep[T]]): Rep[Table[T]]
}

trait InputReaderImplOpsStandard extends InputReaderImplOps { this: OptiQLLift with OptiQLExp =>

  def optiql_table_record_parser_impl[T<:Record:Typ](record: Rep[String], separator: Rep[String]) = {
    val fields = record.split(separator,-1)
    createRecord(fields)
  }

  private def createRecord[T<:Record:Typ](record: Rep[Array[String]]) = {
  ...
  //TODO: this is a map
  def optiql_table_from_seq_impl[T:Typ](elems: Seq[Rep[T]]) = {
