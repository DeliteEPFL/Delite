...
trait DeliteGenTaskGraph extends DeliteCodegen with DeliteKernelCodegen with LoopFusionOpt with LoopSoAOpt {
  ...
  private def emitAnyNode(sym: List[Sym[Any]], rhs: Any): Unit = {
      ...
      val tpes = for (s <- sym) {
        try {
          val tpeStr = gen match {
            case g:ScalaCodegen { val IR: DeliteGenTaskGraph.this.IR.type } if resultIsVar => "generated.scala.Ref[" + g.remap(s.tp) + "]"
            case g:CLikeCodegen { val IR: DeliteGenTaskGraph.this.IR.type } if resultIsVar && cppMemMgr == "recnt" => g.wrapSharedPtr(g.deviceTarget.toString + "Ref" + g.unwrapSharedPtr(g.remap(sym.head.tp)))
            case g:CLikeCodegen { val IR: DeliteGenTaskGraph.this.IR.type } if resultIsVar => g.deviceTarget.toString + "Ref" + g.remap(sym.head.tp)
            case g => g.remap(s.tp)
          }
    ...
    val defs = rhs match {
      ...
      case op:AbstractFatIfThenElse => {
        implicit def anyTyp: Typ[Any] = ManifestTyp(Manifest.Any)
        (op.thenp zip op.elsep) map { case(thenp,elsep) => IfThenElse(op.cond,thenp,elsep)}
      }
  ...
  override def emitBlockHeader(args: List[Sym[Any]], appName: String) {
    ...
    def nestedManifests(m: Typ[_]): List[Typ[_]] = {
    ...
    for (arg <- args) {
      for (gen <- generators) {
        gen match {
          case g: CLikeCodegen { val IR: DeliteGenTaskGraph.this.IR.type } => nestedManifests(arg.tp) foreach { m =>
            try {
