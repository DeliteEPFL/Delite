...
trait DeliteMapOps extends Base {
  ...
  implicit def deliteMapTyp[K:Typ,V:Typ]: Typ[DeliteMap[K,V]]

  object DeliteMap {
    def apply[A:Typ,K:Typ,V:Typ](
    ...
    def apply[A:Typ,K:Typ,V:Typ](coll: Rep[DeliteCollection[A]], keyFunc: Rep[A] => Rep[K], values: Rep[DeliteArray[V]]) = dmap_fromCollection(coll,keyFunc,values)
  }

  implicit def repDMapToDMapOps[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]]) = new DeliteMapOpsCls(dm)

  class DeliteMapOpsCls[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]]) {
  ...
  def dmap_fromCollection[A:Typ,K:Typ,V:Typ](in: Rep[DeliteCollection[A]], keyFunc: Rep[A] => Rep[K], valFunc: Rep[A] => Rep[V], conflictRes: (Rep[V],Rep[V]) => Rep[V]): Rep[DeliteMap[K,V]]
  def dmap_fromCollection[A:Typ,K:Typ,V:Typ](in: Rep[DeliteCollection[A]], keyFunc: Rep[A] => Rep[K], values: Rep[DeliteArray[V]]): Rep[DeliteMap[K,V]]
  def dmap_size[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]])(implicit ctx: SourceContext): Rep[Int]
  def dmap_get[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]], key: Rep[K])(implicit ctx: SourceContext): Rep[V]
  def dmap_contains[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]], key: Rep[K])(implicit ctx: SourceContext): Rep[Boolean]
  def dmap_keys[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]])(implicit ctx: SourceContext): Rep[DeliteArray[K]]
  def dmap_values[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]])(implicit ctx: SourceContext): Rep[DeliteArray[V]]
  //def dmap_toArray[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]])(implicit ctx: SourceContext): Rep[DeliteArray[(K,V)]]
...
trait DeliteMapOpsExp extends DeliteMapOps with DeliteStructsExp { this: DeliteOpsExp =>
  ...
  implicit def deliteMapTyp[K:Typ,V:Typ]: Typ[DeliteMap[K,V]] = {
    implicit val ManifestTyp(m) = typ[DeliteMap[K,V]] // TODO not the same as ArrayOps
    manifestTyp
  }

  trait DeliteIndex[K]
  implicit def deliteIndexTyp[T:Typ]: Typ[DeliteIndex[T]] = {
    implicit val ManifestTyp(m) = typ[T]
    manifestTyp
  }

  case class DeliteIndexGet[K:Typ](index: Rep[DeliteIndex[K]], key: Rep[K]) extends DefWithManifest[K,Int]

  case class DeliteMapNewImm[K:Typ,V:Typ](keys: Rep[DeliteArray[K]], values: Rep[DeliteArray[V]], index: Rep[DeliteIndex[K]], size: Rep[Int]) extends DeliteStruct[DeliteMap[K,V]] {
    val elems = copyTransformedElems(scala.collection.Seq("keys" -> keys, "values" -> values, "index" -> index, "size" -> size))
    val mK = manifest[K]
    val mV = manifest[V]
  }

  case class DeliteMapValues[A:Typ,K:Typ,V:Typ](in: Exp[DeliteCollection[A]], keyFunc: Exp[A] => Exp[K], valFunc: Exp[A] => Exp[V], reduceFunc: (Exp[V],Exp[V]) => Exp[V])
    ...
    def zero = unit(null)(nullTyp).asInstanceOf[Exp[V]]
  }
  ...
  case class DeliteMapKeys[A:Typ,K:Typ](in: Exp[DeliteCollection[A]], keyFunc: Exp[A] => Exp[K])
    extends DeliteOpMappedGroupByReduce[A,K,K,DeliteArray[K]] {

    def alloc(len: Exp[Int]) = DeliteArray[K](len)
    val size = copyTransformedOrElse(_.size)(dc_size(in))
    def zero = unit(null)(nullTyp).asInstanceOf[Exp[K]]
    def valFunc = keyFunc
    def reduceFunc = (a,b) => a
  }

  case class DeliteMapBuildIndex[A:Typ,K:Typ](in: Exp[DeliteCollection[A]], keyFunc: Exp[A] => Exp[K])
  ...
  def dmap_fromCollection[A:Typ,K:Typ,V:Typ](coll: Exp[DeliteCollection[A]], keyFunc: Exp[A] => Exp[K], valFunc: Exp[A] => Exp[V], conflictRes: (Exp[V],Exp[V]) => Exp[V]) = {
    dmap_fromCollection(coll, keyFunc, reflectPure(DeliteMapValues(coll,keyFunc,valFunc,conflictRes)))
  }

  def dmap_fromCollection[A:Typ,K:Typ,V:Typ](coll: Rep[DeliteCollection[A]], keyFunc: Rep[A] => Rep[K], values: Rep[DeliteArray[V]]) = {
  ...
  def dmap_keys[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]])(implicit ctx: SourceContext) = field[DeliteArray[K]](dm, "keys")
  def dmap_values[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]])(implicit ctx: SourceContext) = field[DeliteArray[V]](dm, "values")
  def dmap_size[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]])(implicit ctx: SourceContext) = field[Int](dm, "size")
  protected def dmap_index[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]])(implicit ctx: SourceContext) = field[DeliteIndex[K]](dm, "index")
  def dindex_get[K:Typ](index: Rep[DeliteIndex[K]], key: Rep[K])(implicit ctx: SourceContext): Rep[Int] = reflectPure(DeliteIndexGet(index, key))

  /*def dmap_toArray[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]])(implicit ctx: SourceContext) = {
    dmap_keys(dm).zip(dmap_values(dm)){ (k,v) => t2(k,v) } //note: this should be eliminated if SoA transformations are enabled
  }*/

  def dmap_get[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]], key: Rep[K])(implicit ctx: SourceContext) = {
    val idx = dindex_get(dmap_index(dm), key)
    dmap_values(dm).apply(idx)
  }

  def dmap_contains[K:Typ,V:Typ](dm: Rep[DeliteMap[K,V]], key: Rep[K])(implicit ctx: SourceContext) = {
  ...
  override def mirror[A:Typ](e: Def[A], f: Transformer)(implicit ctx: SourceContext): Exp[A] = (e match {
    ...
    case Reflect(e@DeliteIndexGet(i,k), u, es) => reflectMirrored(Reflect(DeliteIndexGet(f(i),f(k))(e.mA), mapOver(f,u), f(es)))(mtype(manifest[A]), ctx)
